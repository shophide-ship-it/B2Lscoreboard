...
function updateStandings($pdo)
{
    ...
    foreach ($teams as $team) {
        $tid = $team['id'];
        $w = (int)$pdo->query("SELECT COUNT(*) FROM games WHERE status='finished' AND ((home_team_id=$tid AND home_score>away_score) OR (away_team_id=$tid AND away_score>home_score))")->fetchColumn();
        ...
        $pdo->prepare("INSERT INTO standings (team_id, division, season, wins, losses, win_pct, points_for, points_against, streak) VALUES (?, ?, '2024-25', ?, ?, ?, ?, ?, ?) 
            ON DUPLICATE KEY UPDATE wins=?, losses=?, win_pct=?, points_for=?, points_against=?, streak=?")
            ->execute([$tid, $team['division'], $w, $l, $wp, $pf, $pa, $streak, $w, $l, $wp, $pf, $pa, $streak]);
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $act = $_POST['action'] ?? '';
    $hs = isset($_POST['home_score']) ? (int)$_POST['home_score'] : null;
    $as = isset($_POST['away_score']) ? (int)$_POST['away_score'] : null;
    $gt = !empty($_POST['game_time']) ? $_POST['game_time'] : null;

    if ($act === 'add') {
        $ot1_id = isset($_POST['official_team1_id']) ? (int)$_POST['official_team1_id'] : null;
        $ot2_id = isset($_POST['official_team2_id']) ? (int)$_POST['official_team2_id'] : null;

        $pdo->prepare("INSERT INTO games (division, home_team_id, away_team_id, game_date, game_time, venue, status, home_score, away_score, official_team1_id, official_team2_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute([
            (int)$_POST['division'],
    ...
    if ($act === 'edit') {
        ...
        $pdo->prepare("UPDATE games SET division=?, home_team_id=?, away_team_id=?, game_date=?, game_time=?, venue=?, status=?, home_score=?, away_score=?, official_team1_id=?, official_team2_id=? WHERE id=?")->execute([
            (int)$_POST['division'],
            (int)$_POST['home_team_id'],
            (int)$_POST['away_team_id'],
            $_POST['game_date'],
...
// すべての試合を取得
$games = $pdo->query("SELECT g.*, ht.name as home_name, at.name as away_name FROM games g JOIN teams ht ON g.home_team_id=ht.id JOIN teams at ON g.away_team_id=at.id ORDER BY g.game_date DESC, g.game_time DESC LIMIT 50")->fetchAll();
...
<body>
    <div class="admin-layout">
        ...
        <main class="admin-main">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="form-row">
                            <div class="form-group"><label>部門</label><select name="division" class="form-control">
                            ...
                            <div class="form-group"><label>ステータス</label><select name="status" class="form-control">
                        ...
                        <div class="form-group">
                            <label>会場</label><input type="text" name="venue" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>オフィシャル①</label>
                            <select name="official_team1_id" class="form-control">
                                <option value="">選択...</option>
                                <?php foreach ($teams as $t): ?>
                                    <option value="<?= $t['id'] ?>"><?= getDivisionName($t['division']) ?> - <?= htmlspecialchars($t['name']) ?></option>
                                <?php endforeach; ?>
                        ...
                        <div class="form-group">
                            <label>オフィシャル②</label>
                            <select name="official_team2_id" class="form-control">
                                <option value="">選択...</option>
                                <?php foreach ($teams as $t): ?>
                                    <option value="<?= $t['id'] ?>"><?= getDivisionName($t['division']) ?> - <?= htmlspecialchars($t['name']) ?></option>
                                <?php endforeach; ?>
            ...
            <!-- 編集モーダル -->
            <div id="egModal" class="modal-overlay">
                <div class="modal" style="max-width:700px">
                    <div class="modal-header">
                        <h3>試合編集</h3><button class="modal-close">&times;</button>
                    </div>
                    <form method="POST">
                        <div class="modal-body">
                            ...
                            <div class="form-row">
                                <div class="form-group"><label>部門</label><select name="division" id="eg-div" class="form-control">
                                        <option value="1">1部</option>
                                        <option value="2">2部</option>
                                        <option value="3">3部</option>
                                    </select></div>
                                ...
                                <div class="form-group"><label>ステータス</label><select name="status" id="eg-st" class="form-control">
                                        <option value="scheduled">予定</option>
                                        <option value="live">LIVE</option>
                                        <option value="finished">終了</option>
                                    </select></div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    <label>オフィシャル①</label>
                                    <select name="official_team1_id" id="eg-off1" class="form-control">
                                        <option value="">選択...</option>
                                        <?php foreach ($teams as $t): ?>
                                            <option value="<?= $t['id'] ?>">
                                                <?= getDivisionName($t['division']) ?> - <?= htmlspecialchars($t['name']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    <label>オフィシャル②</label>
                                    <select name="official_team2_id" id="eg-off2" class="form-control">
                                        <option value="">選択...</option>
                                        <?php foreach ($teams as $t): ?>
                                            <option value="<?= $t['id'] ?>">
                                                <?= getDivisionName($t['division']) ?> - <?= htmlspecialchars($t['name']) ?>
    ...
    <script>
        function editG(g) {
            ...
            document.getElementById('eg-off1').value = g.official_team1_id || '';
            document.getElementById('eg-off2').value = g.official_team2_id || '';
        }
    </script>
</body>
