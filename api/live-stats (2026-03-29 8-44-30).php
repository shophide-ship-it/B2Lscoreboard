    ...
    case 'undo':
        ...
        try {
            ...
            if ($play['player_id']) {
                ...
                if (isset($fieldMap[$play['action_type']])) {
                    $field = $fieldMap[$play['action_type']];
                    $pdo->prepare("
                        UPDATE live_player_stats 
                        SET {$field} = GREATEST({$field} - 1, 0), points = GREATEST(points - ?, 0)
                        WHERE game_id = ? AND player_id = ?
                    ")->execute([$play['points'], $play['game_id'], $play['player_id']]);
                }
            }
            ...
            if ($play['points'] > 0) {
                ...
                $pdo->prepare("UPDATE live_games SET {$scoreField} = GREATEST({$scoreField} - ?, 0) WHERE game_id = ?")
                    ->execute([$play['points'], $play['game_id']]);
            }
            ...
            if ($play['action_type'] === 'foul') {
                ...
                $pdo->prepare("UPDATE live_games SET {$foulField} = GREATEST({$foulField} - 1, 0) WHERE game_id = ?")
                    ->execute([$play['game_id']]);
            }

            // プレイ記録削除
            $pdo->prepare("DELETE FROM play_by_play WHERE id = ?")->execute([$playId]);
